...
@strawberry.type
@dataclass
class CreateRequestWithdrawEntity():
    tx_hash: str = ""
    block_number: int = 0
    block_time: int = 0
    withdrawIndex: int = 0
...
class CreateRequestWithdrawEvent():
    ...
    def to_entity(self) -> CreateRequestWithdrawEntity:
        props = CreateRequestWithdrawEntity.__dataclass_fields__.keys()
        kv = {prop: self.event[prop] for prop in props}
        return CreateRequestWithdrawEntity(**kv)
